...
# Patterns to include in the auto-generated .gitignore
_GITIGNORE_CONTENT = """\
# Auto-generated by menard - do not commit cache files
*.json
*.state
cache/
"""
...
def ensure_menard_dir(repo_root: Path) -> Path:
    """Create .menard directory and ensure it has a .gitignore for cache files.

    This ensures that all cache files (embeddings, import graphs, etc.) are
    automatically gitignored without requiring manual configuration.

    Returns:
        Path to the .menard directory
    """
    menard_dir = repo_root / ".menard"
    menard_dir.mkdir(exist_ok=True)

    gitignore_path = menard_dir / ".gitignore"
...
def get_cache_dir(repo_root: Path) -> Path:
    """Get or create the .menard cache directory with automatic gitignore."""
    return ensure_menard_dir(repo_root)
